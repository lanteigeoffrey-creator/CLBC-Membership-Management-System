...
// Full profile + attendance history for one member — powers the "view profile" pop-out.
app.get('/api/members/:id/profile', requireAuth, (req, res) => {
  const data = load();
  const member = data.members.find((m) => m.id === Number(req.params.id));
  if (!member) return res.status(404).json({ error: 'Member not found' });
  const branch = data.branches.find((b) => b.id === member.branchId);
  const branchSessions = data.sessions.filter((s) => s.branchId === member.branchId);
  const attendedRecords = data.attendance.filter((a) => a.memberId === member.id);
  const attendedSessionIds = new Set(attendedRecords.map((a) => a.sessionId));
  const recentAttendance = attendedRecords
    .map((a) => {
      const s = data.sessions.find((sess) => sess.id === a.sessionId);
      return { title: s ? s.title : 'Deleted service', date: s ? s.date : null, checkedInAt: a.checkedInAt };
    })
  ...
  res.json({
    ...
    totalSessionsInBranch: branchSessions.length,
    attendedCount: attendedSessionIds.size,
    attendanceRate: branchSessions.length ? Math.round((attendedSessionIds.size / branchSessions.length) * 100) : 0,
    recentAttendance
  });
});
...
// ---------- PUBLIC SELF CHECK-IN (no login required) ----------
// Scoped deliberately narrow: only what a phone scanning a QR code needs.
// Looked up by an unguessable random token (not the sequential session id),
// so one service's link can't be used to guess another's.
// Never exposes phone/email/DOB/etc — just enough to find and check in a name.

app.get('/api/public/sessions/:token', (req, res) => {
  const data = load();
  const sessionObj = data.sessions.find((s) => s.publicToken === req.params.token);
  if (!sessionObj) return res.status(404).json({ error: 'Check-in link not found or service has been removed' });
  const branch = data.branches.find((b) => b.id === sessionObj.branchId);
  res.json({
...
app.get('/api/public/sessions/:token/members', (req, res) => {
  const data = load();
  const sessionObj = data.sessions.find((s) => s.publicToken === req.params.token);
  if (!sessionObj) return res.status(404).json({ error: 'Not found' });
  const query = (req.query.search || '').trim().toLowerCase();
  if (query.length < 2) return res.json([]);
  const matches = data.members
    .filter((m) => m.branchId === sessionObj.branchId)
    .filter((m) => `${m.firstName} ${m.lastName}`.toLowerCase().includes(query))
    .slice(0, 8)
    .map((m) => ({ id: m.id, firstName: m.firstName, lastName: m.lastName, group: m.group }));
  res.json(matches);
});

app.post('/api/public/sessions/:token/checkin', (req, res) => {
  const { memberId, visitorName } = req.body;
  const data = load();
  const sessionObj = data.sessions.find((s) => s.publicToken === req.params.token);
  if (!sessionObj) return res.status(404).json({ error: 'Check-in link not found or service has been removed' });
  if (!memberId && !visitorName) {
    return res.status(400).json({ error: 'Select a name or enter a visitor name' });
  }
  if (memberId) {
    const member = data.members.find((m) => m.id === Number(memberId) && m.branchId === sessionObj.branchId);
    if (!member) return res.status(400).json({ error: 'Member not found' });
    const already = data.attendance.find((a) => a.sessionId === sessionObj.id && a.memberId === member.id);
    if (already) return res.status(200).json({ alreadyCheckedIn: true, name: `${member.firstName} ${member.lastName}` });
  }
  const record = {
    id: nextId(data, 'attendance'),
    sessionId: sessionObj.id,
    memberId: memberId ? Number(memberId) : null,
    visitorName: memberId ? null : (visitorName || '').trim().slice(0, 100),
    checkedInAt: new Date().toISOString()
  };
  data.attendance.push(record);
  save(data);
  const member = memberId ? data.members.find((m) => m.id === Number(memberId)) : null;
  res.status(201).json({ ok: true, name: member ? `${member.firstName} ${member.lastName}` : record.visitorName });
});

// Serve the public self check-in page for any /checkin/:token link
app.get('/checkin/:token', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'self-checkin.html'));
});
