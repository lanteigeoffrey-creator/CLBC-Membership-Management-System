...
async function renderMembers() {
  ...
  main.innerHTML = `<div class="page-header">
    ...
    <div class="field" style="max-width:340px; margin-bottom:16px;">
      <input id="member-search" placeholder="Search members by name…" autocomplete="off" />
    </div>
  ...
  document.getElementById('member-search').addEventListener('input', (e) => {
    renderMembersRows(allLoadedMembers.filter((m) =>
      `${m.firstName} ${m.lastName}`.toLowerCase().includes(e.target.value.trim().toLowerCase())
    ));
  });
...
function renderMembersRows(members) {
  const target = document.getElementById('members-table');
  if (!members.length) {
    target.innerHTML = `<div class="empty-state"><div class="glyph">◇</div>No members found.</div>`;
    return;
  }
    ...
    <tbody>${members.map((m) => `
      <tr>
        ...
        <td style="text-align:right;">
          <button class="btn-ghost btn-sm" onclick="openMemberProfileModal(${m.id})">View</button>
...
// Google Drive "open?id=" / "file/d/ID/view" share links don't render as <img> directly —
// convert them to Drive's public thumbnail endpoint, which does.
function driveImageUrl(url) {
  if (!url) return '';
  const match = url.match(/[-\w]{20,}/);
  if (match) return `https://drive.google.com/thumbnail?id=${match[0]}&sz=w300`;
  return url;
}
...
async function openMemberProfileModal(id) {
  const backdrop = document.createElement('div');
  backdrop.className = 'modal-backdrop';
  backdrop.innerHTML = `<div class="modal" style="width:440px; max-height:85vh; overflow-y:auto;"><p style="text-align:center; color:var(--ink-soft);">Loading…</p></div>`;
  document.body.appendChild(backdrop);
  backdrop.addEventListener('click', (e) => { if (e.target === backdrop) backdrop.remove(); });

  let data;
  try {
    data = await api.get(`/api/members/${id}/profile`);
  } catch (e) {
    backdrop.querySelector('.modal').innerHTML = `<p style="color:var(--alert);">${escapeHtml(e.message)}</p>`;
    return;
  }
  const m = data.member;
  const fullName = `${m.firstName} ${m.lastName}`;
  const gifts = (m.spiritualGifts || []).filter(Boolean);

  backdrop.querySelector('.modal').innerHTML = `
    <div style="text-align:center; margin-bottom:16px;">
      ${m.pictureUrl
        ? `<img src="${driveImageUrl(m.pictureUrl)}" alt="${escapeHtml(fullName)}" style="width:88px; height:88px; border-radius:50%; object-fit:cover; border:2px solid var(--line);" onerror="this.outerHTML='<div class=\\'profile-avatar-fallback\\'>${initials(m.firstName, m.lastName)}</div>'" />`
        : `<div class="profile-avatar-fallback">${initials(m.firstName, m.lastName)}</div>`}
      <h3 style="margin:12px 0 2px;">${escapeHtml(fullName)}</h3>
      <span class="tag ${m.category === 'Visitor' ? 'visitor' : ''} ${m.category === 'Not Regularized' ? 'pending' : ''}">${escapeHtml(m.category)}</span>
      <p style="color:var(--ink-soft); font-size:13px; margin:6px 0 0;">${escapeHtml(data.branchName)} · ${escapeHtml(m.group || 'General')}</p>
    </div>

    <div class="profile-detail-grid">
      <div><label>Phone</label><p>${escapeHtml(m.phone || '—')}</p></div>
      <div><label>Email</label><p>${escapeHtml(m.email || '—')}</p></div>
      <div><label>Gender</label><p>${escapeHtml(m.gender || '—')}</p></div>
      <div><label>Date of birth</label><p>${m.dateOfBirth ? fmtDate(m.dateOfBirth) : '—'}</p></div>
      <div><label>Home cell</label><p>${escapeHtml(m.homeCell || '—')}</p></div>
      <div><label>Rhema class</label><p>${escapeHtml(m.rhemaClass || '—')}</p></div>
    </div>
    ...
    ${gifts.length ? `
      <div style="margin-top:12px;">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:6px; margin-top:6px;">
          ${gifts.map((g) => `<span class="tag">${escapeHtml(g)}</span>`).join('')}
        </div>
      </div>` : ''}
    ...
    <div class="card" style="margin-top:18px;">
      ...
      <div style="padding:16px 20px;">
        <div style="display:flex; gap:20px; margin-bottom:14px;">
          <div><div class="stat-num" style="font-size:22px;">${data.attendedCount}</div><div class="stat-label">Attended</div></div>
          <div><div class="stat-num" style="font-size:22px;">${data.totalSessionsInBranch}</div><div class="stat-label">Services logged</div></div>
          <div><div class="stat-num" style="font-size:22px; color:var(--brass);">${data.attendanceRate}%</div><div class="stat-label">Attendance rate</div></div>
        </div>
        ${data.recentAttendance.length ? `
          <label style="margin-bottom:6px;">Recent check-ins</label>
          <div style="max-height:160px; overflow-y:auto;">
            ${data.recentAttendance.map((r) => `
              <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid var(--line); font-size:13px;">
                ...
                <span style="color:var(--ink-soft);">${r.date ? fmtDate(r.date) : ''}</span>
              </div>`).join('')}
          </div>` : `<p style="color:var(--ink-soft); font-size:13px;">No check-ins recorded yet.</p>`}
      </div>
    </div>

    <div class="modal-actions">
      <button class="btn-ghost" id="profile-close">Close</button>
      <button class="btn-primary" id="profile-edit">Edit member</button>
    </div>`;

  backdrop.querySelector('#profile-close').addEventListener('click', () => backdrop.remove());
  backdrop.querySelector('#profile-edit').addEventListener('click', () => {
    backdrop.remove();
  ...
  target.innerHTML = `<table>
    ...
    <tbody>${sessions.map((s) => `
      <tr>
        ...
        <td style="text-align:right;">
          <button class="btn-ghost btn-sm" onclick="openCheckinLinkModal(${s.id})">Self check-in</button>
...
function openCheckinLinkModal(id) {
  const s = lastLoadedSessions.find((x) => x.id === id);
  if (!s || !s.publicToken) { toast('Check-in link unavailable — try reloading Services', true); return; }
  const url = `${window.location.origin}/checkin/${s.publicToken}`;
  ...
  backdrop.innerHTML = `
    <div class="modal" style="width:360px; text-align:center;">
      <h3>Self check-in</h3>
      <p style="font-size:13.5px; color:var(--ink-soft); margin-top:-8px;">
        ${escapeHtml(s.title)} — ${fmtDate(s.date)}<br/>
        People scan this with their phone camera to check themselves in — no login needed.
      </p>
      <img src="${qrSrc}" alt="QR code" style="width:220px; height:220px; margin:14px auto; display:block; border:1px solid var(--line); border-radius:10px;" />
      <div class="field" style="text-align:left;">
        <label>Link</label>
        <input id="ci-link-input" value="${url}" readonly onclick="this.select()" />
      </div>
      <div class="modal-actions" style="justify-content:center;">
        <button class="btn-ghost" id="ci-link-copy">Copy link</button>
        <button class="btn-primary" id="ci-link-close">Done</button>
      </div>
    </div>`;
  document.body.appendChild(backdrop);
  backdrop.addEventListener('click', (e) => { if (e.target === backdrop) backdrop.remove(); });
  backdrop.querySelector('#ci-link-close').addEventListener('click', () => backdrop.remove());
  backdrop.querySelector('#ci-link-copy').addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText(url);
      toast('Link copied');
    } catch {
      backdrop.querySelector('#ci-link-input').select();
